-- =====================================================
-- EKSİK ENERJİ VERİSİ DOLDURMA SCRIPT (OKUNABİLİR VERSİYON)
-- Ar-Ge (id=14) ve Pazarlama (id=15) için eksik kayıtları doldurur
-- =====================================================

-- ADIM 0: ÖNEMLİ NOTLAR
-- =====================================================
-- 1. Bu script sadece eksik kayıtları ekler, var olanları güncellemez
-- 2. Ar-Ge için %90, Pazarlama için %80 çarpanı uygulanır
-- 3. Değerler: (Diğer departmanların kWh/çalışan ortalaması) * çarpan * çalışan_sayısı
-- 4. cost_tl = consumption_kwh * unit_cost
-- 5. co2_kg = consumption_kwh * emission_factor
...
SELECT 
    ...
    -- consumption_kwh = (kWh/çalışan ortalaması) * çarpan * çalışan_sayısı
    ROUND(
        COALESCE(avg_monthly.avg_kwh_per_emp, avg_global.avg_kwh_per_emp, 0) * 
        missing.multiplier * 
        COALESCE(d.employee_count, 1),
        2
    ) AS consumption_kwh,
    
    -- cost_tl = consumption_kwh * unit_cost
    ROUND(
        COALESCE(avg_monthly.avg_kwh_per_emp, avg_global.avg_kwh_per_emp, 0) * 
        missing.multiplier * 
        COALESCE(d.employee_count, 1) * 
...
FROM (
    -- ADIM 2.1: Eksik kombinasyonları bul (departman x ay x enerji_türü)
    SELECT 
...
-- ADIM 2.2: Departman bilgilerini getir (employee_count için)
INNER JOIN departments d ON d.department_id = missing.dept_id

-- ADIM 2.3: Enerji türü bilgilerini getir (unit_cost ve emission_factor için)
INNER JOIN energy_types et ON et.energy_type_id = missing.energy_type_id

-- ADIM 2.4: O ay + enerji_türü için diğer departmanların kWh/çalışan ortalaması
LEFT JOIN (
    SELECT 
        em.energy_type_id,
...
-- ADIM 2.5: Global ortalama (ay bazında veri yoksa kullanılacak)
LEFT JOIN (
    SELECT 
        em.energy_type_id,
        AVG(em.consumption_kwh / NULLIF(d.employee_count, 0)) AS avg_kwh_per_emp
...
WHERE 
    ...
    COALESCE(avg_monthly.avg_kwh_per_emp, avg_global.avg_kwh_per_emp, 0) > 0
    AND COALESCE(d.employee_count, 0) > 0  -- Çalışan sayısı 0'dan büyük olmalı

ON DUPLICATE KEY UPDATE
    -- Var olan kayıtları ASLA güncelleme
...
-- ADIM 4: DETAYLI KONTROL - Ay bazında
-- =====================================================
SELECT 
    d.department_id,
    d.department_name,
    DATE_FORMAT(COALESCE(em.month_key, target_months.month_key), '%Y-%m-01') AS ay,
    et.energy_type_id,
    et.energy_name,
    COALESCE(ROUND(em.consumption_kwh, 2), 0) AS consumption_kwh,
    COALESCE(ROUND(em.cost_tl, 2), 0) AS cost_tl,
    COALESCE(ROUND(em.co2_kg, 2), 0) AS co2_kg,
    CASE 
